...
import { ColumnModel, QueryCellInfoEventArgs, ResizeService } from '@syncfusion/ej2-angular-grids';
// declare var $:any;
...
@Component({
  selector: 'app-productivity-emp',
  templateUrl: './productivity-emp.component.html',
  styleUrls: ['./productivity-emp.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [DayService, WeekService, WorkWeekService, MonthService, AgendaService],
})

export class ProductivityEmpComponent implements OnInit {
  ...
  //  @ViewChild('studytemplate', { static: false }) public studytemplate: any;
  ...
  public headerText=[];
public showTeamMem;
public studyColumns: ColumnModel[];
public desgnColumns: ColumnModel[];
public shopdrawingColumns: ColumnModel[];
...
public rangeData=[{
 ...
 private x: any;
  ...
  private xAxis: any;
  private yAxis: any;
  studyAttributes: { class: string; };
  desgnattributes: { class: string; };
  shopdrawingAttributes: { class: string; };
  utilfromDateValue: string;
  utiltoDateValue: string;
  customiseCell(args: QueryCellInfoEventArgs) {
    // if (args.column.field == 'task_count_design') {

    //         args.cell.classList.add('orange');

    // }
    if (args.column.field == 'task_count_design' ||args.column.field == 'completed_task_count_design' ||args.column.field == 'pending_count_design' ||args.column.field == 'occupied_design'||args.column.field == 'inprogress_task_count_design'||args.column.field == 'budget_hours_design'||args.column.field == 'worked_hours_design' ) {

            args.cell.classList.add('orange');

    }  if (args.column.field == 'task_count_study' ||args.column.field == 'completed_task_count_study' ||args.column.field == 'pending_count_study' ||args.column.field == 'occupied_study'||args.column.field == 'inprogress_task_count_study'||args.column.field == 'budget_hours_study'||args.column.field == 'worked_hours_study' ) {

      args.cell.classList.add('pink');

}  if (args.column.field == 'task_count_shop_drawing' ||args.column.field == 'completed_task_count_shop_drawing' ||args.column.field == 'pending_count_shop_drawing' ||args.column.field == 'occupied_shop_drawing'||args.column.field == 'inprogress_task_count_shop_drawing'||args.column.field == 'budget_hours_shop_drawing'||args.column.field == 'worked_hours_shop_drawing' ) {

  args.cell.classList.add('purple');
...
// }
  ...
  teamMembers=0;
  working=0;
  productiveTime='0 h 0 m';
  prodPercent:any=0;
  employeeWorkforce: any;
  selectedTabText: string='ALL EMPLOYEES';
  teamEmployeeWorkforce: any;
  selectedDeptId: any='';
  deptWorkforce: any;
  showDeptMem: boolean=false;
  showList = true;
  viewDashboard: boolean;
  projectDetailedData: any=[];
  employeeName: any='';
 ...
 public totalOverTimeCount=0;
  ...
  ondate: any;
    constructor(public timesheetService:TimeSheetService,private userService:UserService,private deptService:DepartmentService, private toastr: ToastrService,private router: Router,private empService:EmployeeService,private teamService:TeamService,private spinner:NgxSpinnerService,private projectService:ProjectService,
      ...
      this.studyAttributes = {class: 'studycss'};
      this.desgnattributes = {class: 'designcss'};
      this.shopdrawingAttributes = {class: 'shopdrwncss'};
      this.studyColumns = [
        {
            field: 'task_count_study',
            headerText: 'Task',
            customAttributes : {class: 'studycss'},
            width: 100,
            textAlign: 'Center',
            minWidth: 10,
           // template:this.studytemplate
        },
        {
          field: 'inprogress_task_count_study',
        ...
        {
            field: 'completed_task_count_study',
        ...
        {
          field: 'occupied_study',
          headerText: 'Occupied Days',
          width: 120,
          customAttributes : {class: 'studycss'},
          textAlign: 'Center',
          minWidth: 10
        ...
        {
            field: 'budget_hours_study',
            headerText: 'Budgeted Hrs',
            width: 120,
            customAttributes : {class: 'studycss'},
            textAlign: 'Center',
            minWidth: 10
        ...
        {
            field: 'worked_hours_study',
            headerText: 'Worked Hrs',
            width: 120,
            customAttributes : {class: 'studycss'},
            textAlign: 'Center',
            minWidth: 10
    ...
    this.desgnColumns = [
      {
          field: 'task_count_design',
          headerText: 'Task',
          customAttributes : {class: 'designcss'},

          width: 100,
          textAlign: 'Center',
          minWidth: 10,
         // template:this.designtemplate
      },

      {
        field: 'inprogress_task_count_design',
        headerText: 'In Progress',
        width: 140,
        customAttributes : {class: 'designcss'},
      ...
      {
          field: 'completed_task_count_design',
          headerText: 'Completed',
          width: 140,
          customAttributes : {class: 'designcss'},
  ...
  this.shopdrawingColumns = [
    {
        field: 'task_count_shop_drawing',
        headerText: 'Task',
        customAttributes : {class: 'shopdrwncss'},
        width: 100,
        textAlign: 'Center',
        minWidth: 10,
       // template:this.shop_drawingtemplate
    },
    {
      field: 'inprogress_task_count_shop_drawing',
      headerText: 'In Progress',
      width: 140,
      customAttributes : {class: 'shopdrwncss'},
    ...
    {
        field: 'completed_task_count_shop_drawing',
        headerText: 'Completed',
        width: 140,
        customAttributes : {class: 'shopdrwncss'},
    ...
    {
        field: 'pending_count_shop_drawing',
        headerText: 'Open',
        width: 120,
        customAttributes : {class: 'shopdrwncss'},
    ...
    },
    {
      ...
      field: 'budget_hours_shop_drawing',
      headerText: 'Budgeted Hrs',
      width: 120,
      customAttributes : {class: 'shopdrwncss'},
      textAlign: 'Center',
  ...
  {
      field: 'worked_hours_shop_drawing',
      headerText: 'Worked Hrs',
      width: 120,
      customAttributes : {class: 'shopdrwncss'},
      textAlign: 'Center',
  ...
  teamEmpId=''
    ...
    if(moment(fromDate).format('L')!=moment(toDate).format('L')){
      ...
      is_Week=false;
          ...
          this.activityService.GetEmployeeTasksTimesheetByOrgID(postData).subscribe(
            (data:any)  => {
              let schedulerData=[]
              let timelineData=[]
        ...
        for(var k=0;k<data[i]['timesheetDataModels'][j]['viewLogDataModels'].length;k++){
         ...
         if(data[i].members.length>1){
          ...
          schedulerData.push({
           ...
           Location:data[i]['timesheetSearchLocationViewModel']['geo_address'],
           Employee:data[i]['timesheetDataModels'][j]['emp_name']
    ...
    // this.projectChart.reflow();
      ...
      // this.timelineRelativeTime();
            ...
            public  GetsScreenshotByOrgIDAndDate(fromDate,toDate) {
              ...
              this.activityService.EmployeeScreenshotsPerDateByOrgIDAndDate(postData).subscribe(
                (data:any)  => {
                  let screenShotData=[]
                 if(data.length!=0){
                   this.screenshotData=data

                  this.activityService.getProfileImage(data[0].img_url).subscribe(
                    data => {
...
if(localStorage.getItem('ActivityEmpID')){
  ...
  teamEmpId=''
      ...
      this.activityService.EmployeeProductivityPerDateByOrgIDAndDate(postData).subscribe(
        (data:any)  => {
      ...
      // this.spinner.hide();
        ...
        this.prodRatio=data.productivity_ratio;
...
this.weekChart1.reflow();
    ...
    if(localStorage.getItem('ActivityEmpID')){
      ...
      teamEmpId=''
          ...
          this.activityService.AdminProductivityTimeFrequencyByEmpIDAndDate(postData).subscribe(
            (data:any)  => {
       let prodData=[]
       let idleData=[]
       let ondate;
...
this.spinner.show();
 ...
 if(this.selectedTabText=='ALL EMPLOYEES'){
  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.selectedDate,this.selectedDate)

 }else if(this.selectedTabText=='TEAM'){
  this.WorkforceProductiveByTeamIDAndDate(this.teamValue,this.selectedDate,this.selectedDate)

 }else{
  this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,this.selectedDate,this.selectedDate)

 }
 localStorage.setItem('productivityfromDate',JSON.stringify(this.selectedDate))
...
this.showMonthsContainer=false;
          ...
          if(this.selectedTabText=='ALL EMPLOYEES'){
           ...
           localStorage.setItem('prodfromDate',JSON.stringify(this.selectedDate));
 localStorage.setItem('prodtoDate',JSON.stringify(this.selectedDate));
   localStorage.setItem('productivityfromDate',JSON.stringify(this.selectedDate))
...
//  localStorage.setItem('prodfromDate',JSON.stringify(this.today));
//  localStorage.setItem('prodtoDate',JSON.stringify(this.today));
//  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.today,this.today)

 if(this.selectedTabText=='ALL EMPLOYEES'){
  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.today,this.today)

 }else if(this.selectedTabText=='TEAM'){
  this.WorkforceProductiveByTeamIDAndDate(this.teamValue,this.today,this.today)

 }else{
  this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,this.today,this.today)
...
if(this.selectedTabText=='ALL EMPLOYEES'){
  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.weekEnd,this.weekStart)

 }else if(this.selectedTabText=='TEAM'){
  this.WorkforceProductiveByTeamIDAndDate(this.teamValue,this.weekEnd,this.weekStart)

 }else{
  this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,this.weekEnd,this.weekStart)

 }

//  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.weekEnd,this.weekStart);

 localStorage.setItem('prodfromDate',JSON.stringify(this.weekEnd));
 localStorage.setItem('prodtoDate',JSON.stringify(this.weekStart));
...
// this.dateValue=this.weekEnd;
         ...
         public getMonthsData(){
          ...
          //  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.monthStart,this.monthEnd);
...
this.selectedDate=this.monthStart;
    ...
    this.isRangeActive=false;
    localStorage.setItem('prodfromDate',JSON.stringify(this.monthStart));
localStorage.setItem('prodtoDate',JSON.stringify(this.monthEnd));
if(this.selectedTabText=='ALL EMPLOYEES'){
  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.monthStart,this.monthEnd)

 }else if(this.selectedTabText=='TEAM'){
  this.WorkforceProductiveByTeamIDAndDate(this.teamValue,this.monthStart,this.monthEnd)

 }else{
  this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,this.monthStart,this.monthEnd)
            ...
            public taskDelete(deptId){
              ...
              //  this.searchOptions = { fields: ['dep_name'], operator: 'contains', key: 'Ha', ignoreCase: true };
                ...
                public tabSelected(args: SelectEventArgs): void {
                  ...
                  if(args.selectedItem.innerText!="Team"){
                    // this.showActDiv=false;
                    this.empValue='';
                    this.teamValue='';
                    this.showTeamMem=false;
                  ...
                  if(args.selectedItem.innerText=="ALL EMPLOYEES"){
                    ...
                    this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),JSON.parse(localStorage.getItem('prodfromDate')),JSON.parse(localStorage.getItem('prodtoDate')))
                    this.showDeptMem=false;
                  ...
                  var results=[]
                     ...
                     if(data){
                       ...
                       this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,JSON.parse(localStorage.getItem('prodfromDate')),JSON.parse(localStorage.getItem('prodtoDate')))

                       this.showDeptMem=true;
                ...
                public WorkforceProductiveByOrgIDAndDate(teamid,startdate,enddate){
                  ...
                  this.activityService.WorkforceEmployeeProductiveByOrgIDAndDate(postData).subscribe(
                    (data:any)  => {
                      if(data){
                        this.spinner.hide();

                        this.teamMembers=data.team_members;
                        if(data.total_productive_time!=null){
                          let totalHrsString=data.total_productive_time!=null?data.total_productive_time.split(':'):0;
                          this.productiveTime=(totalHrsString[0]=="00"?'0 h ':(totalHrsString[0] +' h ')) +(totalHrsString[1]=="00"?'0 m':(totalHrsString[1] +' m '));

                        }else{
                          this.productiveTime=('0 h 0 m');

                        }
                        let empWfData=data.employeeWorkforce!=null?data.employeeWorkforce:[]
                        if(data.employeeWorkforce!=null){
                        for(var i=0;i<  data.employeeWorkforce.length;i++){
                          if(data.employeeWorkforce[i].productive_time!=null){
                            let prodHrsString=data.employeeWorkforce[i].productive_time!=null?data.employeeWorkforce[i].productive_time.split(':'):0;
                            empWfData[i].productive_time=(prodHrsString[0]=="00"?'0 h ':(prodHrsString[0] +' h ')) +(prodHrsString[1]=="00"?'0 m':(prodHrsString[1] +' m '));


                          }else{
                            empWfData[i].productive_time=('0 h 0 m');


                          }
                          if(data.employeeWorkforce[i].desktop_time!=null){

                            let deskHrsString=data.employeeWorkforce[i].desktop_time!=null?data.employeeWorkforce[i].desktop_time.split(':'):0;
                            empWfData[i].desktop_time=(deskHrsString[0]=="00"?'0 h ':(deskHrsString[0] +' h ')) +(deskHrsString[1]=="00"?'0 m':(deskHrsString[1] +' m '));



                          }else{
                            empWfData[i].desktop_time=('0 h 0 m');


                          }
                          if(data.employeeWorkforce[i].time_at_work!=null){

                            let workHrsString=data.employeeWorkforce[i].time_at_work!=null?data.employeeWorkforce[i].time_at_work.split(':'):0;
                            empWfData[i].time_at_work=(workHrsString[0]=="00"?'0 h ':(workHrsString[0] +' h ')) +(workHrsString[1]=="00"?'0 m':(workHrsString[1] +' m '));


                          }else{

                            empWfData[i].time_at_work=('0 h 0 m');

                          }
                        }

                      }
              this.employeeWorkforce=empWfData;

                        // this.productiveTime=data.total_productive_time;
                        this.working=data.working;
                        if(data.total_productive_percent){
                          this.prodPercent=data.total_productive_percent.toString().length==1?data.total_productive_percent:parseFloat(data.total_productive_percent).toFixed(2);
              ...
              this.data12=datas.dataSource['json']
                ...
                public WorkforceProductiveByTeamIDAndDate(teamid,startdate,enddate){
                  let postData={
                    id:teamid,
                    //date:this.dateSelect
                    "startDate": moment(startdate).format('L'),
...
console.log('postData',postData);
                  this.activityService.WorkforceEmployeeProductiveByTeamIDAndDate(postData).subscribe(
                    (data:any)  => {
                      if(data){
                        this.spinner.hide();
                        this.teamMembers=data.team_members;
                        ...
                        //this.prodPercent=parseFloat(data.total_productive_percent).toFixed(2);
                        this.prodPercent=data.total_productive_percent.toString().length==1?data.total_productive_percent:parseFloat(data.total_productive_percent).toFixed(2);
                   ...
                   let empWfData=data.employeeWorkforce
                   for(var i=0;i<data.employeeWorkforce.length;i++){
                     if(data.employeeWorkforce[i].productive_time!=null){
                       let prodHrsString=data.employeeWorkforce[i].productive_time!=null?data.employeeWorkforce[i].productive_time.split(':'):0;
                       empWfData[i].productive_time=(prodHrsString[0]=="00"?'0 h ':(prodHrsString[0] +' h ')) +(prodHrsString[1]=="00"?'0 m':(prodHrsString[1] +' m '));
              ...
              this.teamEmployeeWorkforce=empWfData;
                      }
                ...
                public WorkforceProductiveByDeptIDAndDate(teamid,startdate,enddate){
                  let postData={
                    id:teamid,
                    // date:this.dateSelect
                    "startDate": moment(startdate).format('L'),
                  ...
                  this.activityService.WorkforceEmployeeProductiveByDeptIDAndDate(postData).subscribe(
                    (data:any)  => {
                      if(data){
                        this.spinner.hide();
              ...
              // this.data12=datas.dataSource['json']
                ...
                public changedTeam(e: any): void {
                  ...
                  if(e.value!='' &&e.value!='Select' ){
                    this.FetchAllTeamMembersByTeamID(e.value);
                    this.WorkforceProductiveByTeamIDAndDate(e.value,JSON.parse(localStorage.getItem('prodfromDate')),JSON.parse(localStorage.getItem('prodtoDate')));

                    this.showTeamMem=true;
                  }



                }
                public FetchAllEmpByOrgID(teamid){
                  let postData={
                    id:teamid
                  }
              ...
              this.data12=datas.dataSource['json']
                ...
                public FetchAllTeamMembersByTeamID(teamid){
                  let postData={
                    id:teamid
                  }
              ...
              this.data12=datas.dataSource['json']
                ...
                OnDetailClose(){
                  $("#detail_log_modal").modal('hide');
                  this.closeBtn.nativeElement.click();

                }
                toActivitiesEmp(emp_id,full_name){
                  $("#detail_log_modal").modal('show');
                  this.employeeName=full_name;
                  console.log('employeeName',full_name);
this.spinner.show();
                  let postData={
                    id:emp_id,
                   // date:this.dateSelect
                    "startDate": (moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('L')),
                    "endDate":(moment(JSON.parse(localStorage.getItem('prodtoDate'))).format('L'))
                  }

                  this.activityService.ProjectDetailByEmpIDAndDate(postData).subscribe(
                    (data:any)  => {

                      if(data.length!=0){
this.spinner.hide();

                       this.projectDetailedData=data;
                      }else{
this.spinner.hide();

                       this.projectDetailedData=[];
         ...
         public testDataRelative = [
                ...
                customRadioChange(id,type){
    this.spinner.show();

                  if(id=="7"){
                   let dateTo = moment().format('L');
                   let dateFrom = moment().subtract(7,'d').format('L');
                   this.isTodayActive=false;
this.isWeekActive=true;
this.isMonthActive=false;
this.isRangeActive=false;
this.dateText=moment(dateFrom).format('ddd, D MMM YYYY')+' - '+moment(dateTo).format('ddd, D MMM YYYY');
localStorage.setItem('prodfromDate',JSON.stringify(dateFrom));
localStorage.setItem('prodtoDate',JSON.stringify(dateTo));
if(this.selectedTabText=='ALL EMPLOYEES'){
  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),dateFrom,dateTo)

 }else if(this.selectedTabText=='TEAM'){
  this.WorkforceProductiveByTeamIDAndDate(this.teamValue,dateFrom,dateTo)

 }else{
  this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,dateFrom,dateTo)

 }
//  this.dateRangeForm.patchValue({
//   daterange:[new Date(moment(dateFrom).format('L')), new Date(moment(dateTo).format('L'))],

// })

                  }else if(id=="15"){
                    let dateTo = moment().format('L');
                    let dateFrom = moment().subtract(15,'d').format('L');
                    this.isTodayActive=false;
                    this.isWeekActive=false;
                    this.isMonthActive=false;
...
// })
                    ...
                    let dateTo = moment().format('L');
                    let dateFrom = moment().subtract(30,'d').format('L');
                    this.isTodayActive=false;
this.isWeekActive=false;
this.isMonthActive=true;
this.isRangeActive=false;
...
// })
                    ...
                    let dateTo = moment().format('L');
                    let dateFrom = moment().subtract(45,'d').format('L');
                    localStorage.setItem('prodfromDate',JSON.stringify(dateFrom));
                    localStorage.setItem('prodtoDate',JSON.stringify(dateTo));
                    this.isTodayActive=false;
                    this.isWeekActive=false;
...
// })
  ...
  this.rangeForm=new FormGroup({
    ...
    sinceRangePicker: new FormControl(''),
 ...
 // this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),JSON.parse(localStorage.getItem('prodfromDate')),JSON.parse(localStorage.getItem('prodtoDate')))
  if(localStorage.getItem('prodfromDate')){
    if((JSON.parse(localStorage.getItem('prodfromDate')) && JSON.parse(localStorage.getItem('prodtoDate')) ==this.today)){
this.dateText=moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('ddd, D MMM YYYY');

    }else if(localStorage.getItem('prodfromDate')!=localStorage.getItem('prodtoDate')){
this.dateText=moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('ddd, D MMM YYYY')+' - '+moment(JSON.parse(localStorage.getItem('prodtoDate'))).format('ddd, D MMM YYYY');
  ...
  if(JSON.parse(localStorage.getItem('prodfromDate'))!=''){
    this.isTodayActive=true;
 ...
 this.getTodaysData();
  ...
  else if(moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('L')==moment(this.monthStart).format('L')){
    this.spinner.show();

   this.getMonthsData()

  }else if(moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('L')==moment(this.weekEnd).format('L')){
    this.spinner.show();

    this.getWeeksData()

  }
  else if(JSON.parse(localStorage.getItem('prodfromDate'))!=JSON.parse(localStorage.getItem('prodtoDate'))){
    this.spinner.show();
...
if(JSON.parse(localStorage.getItem('userRights')).length!=0){
  ...
  for(var i=0;i<userRights.length;i++){
    if(userRights[i].module_name=='Workforce'){
  ...
  if(userRights[i].section_name=='View Activities Dashboard' && userRights[i].is_allow==true ){
    this.viewDashboard=true
  }
else if(userRights[i].section_name=='View Activities Dashboard' && userRights[i].is_allow==false ){
...
// // this.EmployeeAppTrackedByEmpIDAndDate(this.today,this.today);
// this.GetTimesheetActivityByEmpIDAndDate(this.today,this.today);
// this.GetsScreenshotByOrgIDAndDate(this.today,this.today);
// this.GetTaskTimelineByEmpIDAndDate(this.today,this.today);
...
this.rangeForm.patchValue({
  ...
  this.spinner.show();
  // localStorage.setItem('productivityfromDate',JSON.stringify(startTime[0]))
  localStorage.setItem('prodtoDate',JSON.stringify(startTime[1]));
  // this.GetTimesheetActivityByEmpIDAndDate(this.fromDateValue,this.toDateValue);
  if(this.selectedTabText=='ALL EMPLOYEES'){
    this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.fromDateValue,this.toDateValue)

   }else if(this.selectedTabText=='TEAM'){
    this.WorkforceProductiveByTeamIDAndDate(this.teamValue,this.fromDateValue,this.toDateValue)

   }else{
    this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,this.fromDateValue,this.toDateValue)
...
this.rangeForm.get('sinceRangePicker').valueChanges.subscribe(() => {
  // fires when the input value has actually changed

// this.rangeForm.patchValue({
//   sinceRangePicker:'50'
// })
  let startTime=this.rangeForm.get('sinceRangePicker').value;
  this.utilfromDateValue=moment(startTime[0]).format('L');
  this.utiltoDateValue=moment(startTime[1]).format('L');
  this.spinner.show();
  localStorage.setItem('utilfromDate',JSON.stringify(startTime[0]))
  localStorage.setItem('utiltoDate',JSON.stringify(startTime[1]));


  localStorage.setItem('prodfromDate',JSON.stringify(startTime[0]));
  localStorage.setItem('prodtoDate',JSON.stringify(startTime[1]));
  this.dateText=moment().format('ddd, D MMM YYYY');
  // this.GetTimesheetActivityByEmpIDAndDate(this.fromDateValue,this.toDateValue);

  if(this.selectedTabText=='ALL EMPLOYEES'){
    this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),this.utilfromDateValue,this.utiltoDateValue)

   }else if(this.selectedTabText=='TEAM'){
    this.WorkforceProductiveByTeamIDAndDate(this.teamValue,this.utilfromDateValue,this.utiltoDateValue)

   }else{
    this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,this.utilfromDateValue,this.utiltoDateValue)
...
if(localStorage.getItem('prodfromDate')){
  let dateTo = moment().format('L');
  let dateFrom = moment().subtract(15,'d').format('L');
  let mdateFrom = moment().subtract(30,'d').format('L');

let rdateFrom = moment().subtract(45,'d').format('L');
  if(JSON.parse(localStorage.getItem('prodfromDate'))==(dateFrom) && JSON.parse(localStorage.getItem('prodtoDate'))==dateTo ){
    this.rangeForm.patchValue({
    rangeType:'15'
    })
  }
  else if(moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('L')==moment(this.weekEnd).format('L') && moment(JSON.parse(localStorage.getItem('prodtoDate'))).format('L')==moment(this.weekStart).format('L') ){
this.rangeForm.patchValue({
rangeType:'7'
})

  }
else if(JSON.parse(localStorage.getItem('prodfromDate'))==(mdateFrom) && JSON.parse(localStorage.getItem('prodtoDate'))==dateTo ){
  this.rangeForm.patchValue({
  rangeType:'30'
  })

  this.getMonthsData();
}else if(JSON.parse(localStorage.getItem('prodfromDate'))==(rdateFrom) && JSON.parse(localStorage.getItem('prodtoDate'))==dateTo ){
  this.rangeForm.patchValue({
  rangeType:'45'
  })
}
...
if(localStorage.getItem('utilfromDate')){
  this.rangeForm.patchValue({
    sinceRangePicker:[JSON.parse(localStorage.getItem('utilfromDate')),JSON.parse(localStorage.getItem('utiltoDate'))]
  })
 ...
 localStorage.setItem('productivityfromDate',JSON.stringify(dateValue))
 console.log('datevalueChanges',this.datePickerForm.get('date').value);
//  sessionStorage.setItem('workforceDate',JSON.stringify(dateValue));
 if(this.selectedTabText=='ALL EMPLOYEES'){
  this.WorkforceProductiveByOrgIDAndDate(localStorage.getItem('org_id'),dateValue,dateValue)

 }
 else if(this.selectedTabText=='TEAM'){
  this.WorkforceProductiveByTeamIDAndDate(this.teamValue,dateValue,dateValue)

 }else{
  this.WorkforceProductiveByDeptIDAndDate(this.selectedDeptId,dateValue,dateValue)
...
console.log('isTodayActive',this.isTodayActive)
if(localStorage.getItem('prodfromDate')){
  if((JSON.parse(localStorage.getItem('prodfromDate')) && JSON.parse(localStorage.getItem('prodtoDate')) ==this.today)){
this.dateText=moment(JSON.parse(localStorage.getItem('prodfromDate'))).format('ddd, D MMM YYYY');
...
if(localStorage.getItem('productivityfromDate')){
  if((JSON.parse(localStorage.getItem('productivityfromDate')) )){
this.dateText=moment(JSON.parse(localStorage.getItem('productivityfromDate'))).format('ddd, D MMM YYYY');
let dateValue=moment(JSON.parse(localStorage.getItem('productivityfromDate'))).format('L')
