...
@Component({
  selector: 'app-cost-specification',
  templateUrl: './cost-specification.component.html',
  styleUrls: ['./cost-specification.component.scss']
})

export class CostSpecificationComponent implements OnInit {
...
public maxRangeDate: Date = this.today;
  milestoneTaskValue: any='';
  staticBtnClick: boolean;
  ...
  updateDaterange(){
    ...
    this.getAllStaticMilestoneTasks(this.milestoneValue);

  }
  public changedMilestone(e) {
    console.log('changedMilestone',e.value)
    ...
    // }
    if(e.value!=''){
      this.milestoneTaskValue= e.value;

    }

    if(e.value!='' && e.data && e.data[0].additional.start!=null && e.data[0].additional.end!=null){
    this.milestoneValue= e.value;

      this.dateRangeForm.patchValue({
        daterangAvgTime:[new Date(moment(e.data[0].additional.start).format('L')), new Date(moment(e.data[0].additional.end).format('L'))],

      })
     //this.getAllStaticMilestoneTasks(e.value);
      this.GetAllStaticMilestoneByMilestoneID(e.value);

    }else if(e.value!='' && e.data && e.data[0].additional.start==null && e.data[0].additional.end==null){
    this.milestoneValue= e.value;

      this.dateRangeForm.patchValue({
        daterangAvgTime:[e.data[0].additional.start, e.data[0].additional.end],
...
this.getAllStaticMilestone(null);
...
for (var i = 0; i < emails.value.length; i++) {
      ...
      taskTypedata.push({
          ...
          "id": emails.value[i].id,
          "project_id": null,
          "milestone_id":null,
          "task_name": emails.value[i].Label,
          "unit": emails.value[i].UnitType=='2'?'Unit':(emails.value[i].UnitType=='3'?'Project':'Floors'),
          "qty": emails.value[i].unit_no,
...
if(this.emailForm.get('emails').status=='VALID' && this.milestoneTaskValue!=''){

  this.spinner.show();
     
        return this.costService.UpdateStaticCostTask(taskTypedata).subscribe(
          (data:any)  => {
            // let dataObj = JSON.parse(data['token']);
    ...
    design_name: this.taskForm.get('taskName').value  ,
      ...
      if(this.taskForm.get('taskName').value!='' ){
        this.spinner.show();
          ...
          return this.costService.UpdateTypeOfDesign(postData).subscribe(
            (data:any)  => {
              // let dataObj = JSON.parse(data['token']);
...
this.taskForm.reset();
    ...
    public  getAllStaticMilestone(value){
      this.costService.GetAllStaticMilestoneByOrgID().subscribe(
        
        (data:any) => {
    
      var results=[{ id: '', text: 'Select an option',additional:{start:'',end:''}  }]
          // let dataObj = JSON.parse(data['token']);
        //  
    ...
    results.push({
        ...
        additional:{start:data[i].start_date,end:data[i].end_date} 
    ...
    console.log('milestoneTaskValue',value)
    if(value!=null){
      this.milestoneTaskValue=value;
    }
        // this.router.navigate(["/organizations"]);
        ...
        GetAllStaticMilestoneByMilestoneID(milesoneId){
          this.spinner.show();
          let postData={
            id:milesoneId
           
          }
          this.costService.GetAllStaticMilestoneByMilestoneID(postData).subscribe(
         ...
         if(data){
          this.dateRangeForm.patchValue({
            daterangAvgTime:[data.start_date!='Invalid date'?data.start_date:null,data.end_date!='Invalid date'?data.end_date:null],
    
          })
         this.spinner.hide();
      this.getAllStaticMilestoneTasks(milesoneId);
        ...
        public  getAllStaticMilestoneTasks(milesoneId){
          let startTime=this.dateRangeForm.get('daterangAvgTime').value;
        this.staticBtnClick=true;
          let postData={
            id:milesoneId,
            "orgID": localStorage.getItem('org_id'),
            "startDate":startTime[0]!=null?moment(startTime[0]).format('L'):null,
            "endDate": startTime[1]!=null?moment(startTime[1]).format('L'):null
          }
if(this.milestoneTaskValue!=''){
  this.spinner.show();
        ...
        this.staticBtnClick=false;

         // this.getAllStaticMilestone(this.milestoneValue);

          // this.milestoneValue=milesoneId;
          console.log('milestoneValue',this.milestoneValue);
    ...
    public  getAllTask(){
      this.taskService.getAllTask().subscribe(
        ...
        this.teamAdd=false;
          ...
          this.costService.FindByTypeOfDesignID(postData).subscribe(
            (data:any)  => {
             
           
              this.taskForm.patchValue({
           
                taskName: data.design_name,
  ...
  this.emptoolbar = ['Search' ];
          ...
          let unitType='';
                ...
                results.push({ Label: taskList[i].task_name, id: taskList[i].id,UnitType: unitType,unit_no:taskList[i].qty,avg_time:taskList[i].actual_avg_hrs })
  ...
  ngOnInit() {
    ...
    this.dateRangeForm = new FormGroup({
      daterangAvgTime: new FormControl(''),
