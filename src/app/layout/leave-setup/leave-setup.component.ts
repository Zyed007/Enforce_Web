...
export class LeaveSetupComponent implements OnInit {
  ...
  public leaveTypeData: Array<Select2OptionData>;
  ...
  public changedLeaveType(e: any): void {
    this.leavetypeValue= e.value;
...
public onAddSubmit(){
  ...
  let postData={
    ...
    leave_name: this.statusForm.get('statusName').value  ,
    leave_type_id: this.leavetypeValue,
    max_leave_days: this.statusForm.get('leaveNo').value,
    ...
    if (this.statusForm.status== 'VALID') {
        ...
        return this.leaveService.AddLeaveSetup(postData).subscribe(
          (data:any)  => {
            // let dataObj = JSON.parse(data['token']);
          
          if(data.status==200){
this.onAddTimeofftypeSubmit();
...
results.push({
    "id": data[i].id,
    "text": data[i].leave_type_name
});

}

    
this.leaveTypeData =results;
...
//.dataSource=ds1.dataSource['json'];
        ...
        this.EmpList();
          ...
          this.leaveService.FindByLeaveSetupID(postData).subscribe(
            (data:any)  => {
             ...
             let postData={
              ...
              this.statusForm.patchValue({
                ...
                statusName: data.leave_name,
                leaveNo:data.max_leave_days,
               
              
              })
              this.leavetypeValue=data.leave_type_id;
              // this.DesgnList();
  ...
  this.addformSubmitted=true;
    ...
    max_leave_days: this.statusForm.get('leaveNo').value,
            ...
            if (this.statusForm.status== 'VALID') {
                  ...
                  return this.leaveService.UpdateLeaveSetup(postData).subscribe(
                    (data:any)  => {
                      // let dataObj = JSON.parse(data['token']);
  ...
  this.editable=false;
        ...
        public taskDelete(deptId){
          ...
          this.leaveService.RemoveLeaveSetup(postData).subscribe(
            (data:any)  => {
  ...
  ngOnInit() {
    ...
    this.statusForm = new FormGroup({
      ...
      leaveNo: new FormControl('',[Validators.required]),
