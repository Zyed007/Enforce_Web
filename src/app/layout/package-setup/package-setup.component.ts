...
@Component({
  ...
  styleUrls: ['./package-setup.component.scss']
})
...
export class PackageSetupComponent implements OnInit {
  ...
  constructor(public taskService:TaskService,private toastr: ToastrService,public costService:CostService, private formBuilder: FormBuilder,public empService:EmployeeService,
    ) { 
    this.taskPriority = '0';
    this.taskStatus = '0';
...
public onAddSubmit(){
  ...
  let postData={
    ...
    package_name: this.statusForm.get('statusName').value  ,
    ...
    if (this.statusForm.status== 'VALID') {
        return this.costService.AddPackages(postData).subscribe(
          (data:any)  => {
...
this.assigneeData =results;
    ...
    public  getAllTaskStatus(){
      this.taskService.getAllTaskStatus().subscribe(
        ...
        this.showAddForm=true;
          ...
          this.costService.FindByPackagesID(postData).subscribe(
            (data:any)  => {
             
             
              this.statusForm.patchValue({
           
                statusName: data.package_name,
    ...
    if (this.statusForm.status== 'VALID') {
                  ...
                  return this.costService.UpdatePackages(postData).subscribe(
                    (data:any)  => {
                      // let dataObj = JSON.parse(data['token']);
