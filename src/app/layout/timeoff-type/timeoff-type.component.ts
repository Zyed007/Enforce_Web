...
@Component({
  selector: 'app-timeoff-type',
  templateUrl: './timeoff-type.component.html',
  styleUrls: ['./timeoff-type.component.scss']
})
...
export class TimeoffTypeComponent implements OnInit {
  ...
  leaveSetupData: { id: string; text: string; }[];
  leavesetupValue: any='';
  leavesetupValueTxt: any;
  constructor(public taskService:TaskService,private toastr: ToastrService,private leaveService:LeaveService,  private formBuilder: FormBuilder,public empService:EmployeeService,
    ) {
    this.taskPriority = '0';
    this.taskStatus = '0';
  ...
  changedLeaveSetup(e: any): void {
    this.leavesetupValue= e.value;
    if(e.value!='' && e.data && e.data[0].text){
      this.leavesetupValueTxt=e.data[0].text;
    }
...
results.push({
    "id": data[i].id,
    "text": data[i].leave_name
});

}


this.leaveSetupData =results;
...
public onAddSubmit(){
  ...
  let postData={
    ...
    timeoff_type_name: this.leavesetupValueTxt,
    timeoff_type_earned: this.statusForm.get('desc').value,



    }
    console.log('leavesetupValueTxt',this.leavesetupValueTxt);

    if (this.statusForm.status== 'VALID') {

        return this.leaveService.AddTimeOffSetup(postData).subscribe(
          (data:any)  => {
            // let dataObj = JSON.parse(data['token']);
...
//.dataSource=ds1.dataSource['json'];
        ...
        this.showAddForm=true;
          ...
          this.leaveService.FindByTimeOffSetupID(postData).subscribe(
            (data:any)  => {


              this.statusForm.patchValue({

                statusName: data.timeoff_type_name,
                desc:data.timeoff_type_earned,
  ...
  this.addformSubmitted=true;
          ...
          let postData={
            ...
            timeoff_type_name: this.statusForm.get('statusName').value  ,
            timeoff_type_earned: this.statusForm.get('desc').value,
            ...
            if (this.statusForm.status== 'VALID') {
                  ...
                  return this.leaveService.UpdateTimeOffSetup(postData).subscribe(
                    (data:any)  => {
                      // let dataObj = JSON.parse(data['token']);
  ...
  this.editable=false;
        ...
        public taskDelete(deptId){
          ...
          this.leaveService.RemoveTimeOffSetup(postData).subscribe(
            (data:any)  => {
